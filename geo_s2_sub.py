"""
Advanced Geodesic Pattern Learning v2 - SUBSET VERSION

This is a subset version of geo_s2.py designed to work with data from
saliency_clean_export_sub.py (first n classes, default 20).

Key differences from geo_s2.py:
- Default num_classes: 20 (instead of 1000)
- Default data_dir: Points to subset output format
- Optimized for faster prototyping with smaller datasets
...
Usage:
    # With edge-aware gating (default, recommended) - 20 classes
    python geo_s2_sub.py --data_dir ./data/saliency_imagenet_sub_c20_s100 \
                         --num_classes 20 --k_subpatterns 10 --epochs 50 --use_edge_gating

    # Custom subset (e.g., 50 classes)
    python geo_s2_sub.py --data_dir ./data/saliency_imagenet_sub_c50_s50 \
                         --num_classes 50 --k_subpatterns 10 --epochs 50

    # Without edge-aware gating (fallback)
    python geo_s2_sub.py --data_dir ./data/saliency_imagenet_sub_c20_s100 \
                         --num_classes 20 --k_subpatterns 10 --epochs 50
"""
...
class AdvancedLDDMM_Pipeline(nn.Module):
    ...
    def __init__(self, num_classes=1000, k_subpatterns=10, img_res=(224, 224),
        ...
        print(f"\nAdvanced LDDMM Pipeline v2 Initialized (SUBSET VERSION):")
...
class AdvancedLDDMMTrainer:
    ...
    def __init__(self, model, train_loader, val_loader=None,
        ...
        self.criterion = AdvancedLDDMMLoss(
            ...
            lambda_compactness=15.0,              # STRONG: Prefer blob-like over line-like
    ...
    def train(self, num_epochs, save_frequency=5):
        ...
        print(f"\n{'='*80}")
        print(f"Starting Advanced LDDMM Training (SUBSET VERSION)")
        print(f"{'='*80}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Advanced Geodesic Pattern Learning v2 - SUBSET VERSION',
        epilog="""
SUBSET VERSION - For Fast Prototyping:
  - Default num_classes: 20 (instead of 1000)
  - Works with data from saliency_clean_export_sub.py
...
RECOMMENDED USAGE:
  python geo_s2_sub.py --data_dir ./data/saliency_imagenet_sub_c20_s100 \\
                       --num_classes 20 --use_ot --use_edge_gating --epochs 50
        """
    )
    parser.add_argument('--data_dir', type=str,
                       default='./data/saliency_imagenet_sub_c20_s100',
                       help='Directory with saliency maps (default: c20_s100 subset)')
    parser.add_argument('--num_classes', type=int, default=20,
                       help='Number of classes (default: 20 for subset)')
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='./checkpoints_sub',
                       help='Checkpoint directory (default: ./checkpoints_sub)')
    args = parser.parse_args()

    # Setup
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n{'='*80}")
    print(f"Advanced Geodesic Pattern Learning v2 - SUBSET VERSION")
    print(f"{'='*80}")
    ...
    trainer.plot_training_curves(
        save_path=Path(args.checkpoint_dir) / 'training_curves_sub.png'
    )
    ...
    print(f"  python geo_s2_sub.py --data_dir {args.data_dir} --num_classes {args.num_classes} --use_ot --use_edge_gating --epochs {args.epochs}")
