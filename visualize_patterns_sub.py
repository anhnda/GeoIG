"""
Pattern Visualization for Geodesic LDDMM Model - SUBSET VERSION

This script visualizes patterns from geo_s2_sub.py outputs:
...
Key differences from visualize_patterns.py:
- Default paths point to subset data directories
- Works with smaller class counts (default: 20)
- Optimized for fast prototyping

Usage:
    # Visualize class 0 patterns (subset version)
    python visualize_patterns_sub.py --checkpoint checkpoints_sub/advanced_lddmm_model_final.pth --class_id 0

    # Visualize specific class with custom subset data
    python visualize_patterns_sub.py --checkpoint checkpoints_sub/advanced_lddmm_model_epoch_10.pth \
                                      --class_id 5 \
...
class PatternVisualizerSubset:
    """Visualize learned patterns and decompositions for subset models."""
    ...
    def __init__(self, checkpoint_path, data_dir, device='cuda', imagenet_dir=None, images_per_class=100):
        ...
        print(f"Loaded model (SUBSET VERSION):")
        ...
        # Get class names (only first N classes for subset)
        self.class_names = {idx: name for idx, (wnid, name) in enumerate(list(IMAGENET2012_CLASSES.items())[:self.num_classes])}

        # Build or load class index for fast sample lookup
        self.cache_file = self.data_dir / "class_index_cache_sub.pkl"
        ...
        for batch_file, item_indices in batch_to_indices.items():
            ...
            for item_idx in item_indices:
                ...
                if 'rgb_image' in item:
                    sample_dict['rgb_image'] = item['rgb_image']
    ...
    def get_original_image(self, sample):
        """
        Get original image from sample - from rgb_image field in subset data.
        ...
        """
        # Method 1: RGB image stored directly (from subset export with edge gating)
        if 'rgb_image' in sample:
            rgb_array = sample['rgb_image']
            # If it's in CHW format, convert to HWC
        ...
        # Method 2: Image already stored in normalized format (backward compatibility)
    ...
    def visualize_decomposition(self, class_id, sample_idx=0, save_path=None):
        ...
        # Title
        fig.suptitle(f'Pattern Decomposition (SUBSET): {class_name} (Class {class_id})\n'
    ...
    def compare_samples(self, class_id, num_samples=6, save_path=None):
        ...
        fig.suptitle(f'Sample Comparison (SUBSET): {class_name} (Class {class_id})',
                    fontsize=16, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize learned patterns from LDDMM model (SUBSET VERSION)'
    )
    parser.add_argument('--checkpoint', type=str, default='checkpoints_sub/advanced_lddmm_model_final.pth',
                       help='Path to model checkpoint (default: subset checkpoint)')
    parser.add_argument('--data_dir', type=str,
                       default='./data/saliency_imagenet_sub_c20_s100',
                       help='Directory with saliency maps (default: subset data)')
    ...
    parser.add_argument('--output_dir', type=str, default='./visualizations_sub',
                       help='Output directory for saved figures')
    parser.add_argument('--device', type=str, default='cuda',
                       help='Device to use (cuda/cpu)')
    parser.add_argument('--imagenet_dir', type=str, default=None,
                       help='Path to ImageNet directory (optional)')
    ...
    # Create visualizer
    print(f"\n{'='*80}")
    print(f"Pattern Visualization (SUBSET VERSION)")
    print(f"{'='*80}")

    visualizer = PatternVisualizerSubset(
    ...
    patterns_path = output_dir / f"class_{args.class_id}_patterns_sub.png"
    ...
    decomp_path = output_dir / f"class_{args.class_id}_decomposition_sample_{args.sample_idx}_sub.png"
    ...
    comparison_path = output_dir / f"class_{args.class_id}_comparison_sub.png"
